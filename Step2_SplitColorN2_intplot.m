...
SkipData=10; %Use 1 for non-skip
...
outfile = sprintf('%s_Split.bin',filehead);
...
%     MergedList(NumofMatch,:)=[SplitData.cat(i) MergedX MergedY MergedX MergedY Left.h(i) Left.area(i) Left.width(i) ComponentLX ComponentLY ZDiff MergedI Left.frame(i) Left.length(i) Left.link(i) Left.valid(i) MergedZ MergedZ];
    ...
    if IntensityRatio>IntensityRatioTh
        SplitData.cat(i)=0;
%     elseif IntensityRatio<IntensityRatioTh & IntensityRatio>IntensityRatioLower
%          SplitData.cat(i)=3;
    else
        SplitData.cat(i)=1;
    end
...
a=25;
% figure(1)
% plot(RightIntensity,LeftIntensity,'.','MarkerSize',4)
% xlim([0 70000])
% ylim([0 70000])
% xlabel('Intensity (left)')
% ylabel('Intensity (right)')
% (RightIntensity+LeftIntensity/numel(RightIntensity));
